...
```
```{r delays weighted by number of flights, cache=TRUE, fig.height=12, evaluate=FALSE, fig.width=8, include=FALSE}
avg_delay_w_count <- slim_data2 %>%
  ...
  as.data.frame() %>%
  as_tsibble(key=c("dest_state_nm"))
...
#just looking at western states again since California has the most flights
western_wt_no <- wt_delay_by_no[wt_delay_by_no$dest_state_nm %in% western, ]
...
p_west <- ggplot(western_wt_no %>% filter(dest_state_nm!="Wyoming"), 
                 aes(x = `wt_delay`, y = reorder(`dest_state_nm`, wt_delay, sd), 
                     fill = pandemic)) +
  geom_density_ridges(alpha=.8, scale = 2, rel_min_height = 0.01) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
